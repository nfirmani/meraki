...
async function authenticate(scopes) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let oAuth2Client = new googleapis_1.google.auth.OAuth2(key.client_id, key.client_secret, key.redirect_uris[0]);
            try {
...
let MyServiceImpl = class MyServiceImpl {
    async getDocTitle() {
        return new Promise((resolve, reject) => {
            authenticate(SCOPES)
                .then(client => {
                const docs = googleapis_1.google.docs({ version: 'v1', auth: client });
                docs.documents.get({
...
*/
            data_source_1.AppDataSource.initialize().then(async (dataSource) => {
                let postRepository = dataSource.getRepository(Utente_1.Utente);
                let utente1 = new Utente_1.Utente();
                utente1.firstName = "Evangelista";
                utente1.lastName = "Claudio";
                utente1.age = 30;
                let utente2 = new Utente_1.Utente();
                utente2.firstName = "Fidanza";
                utente2.lastName = "Erika";
                utente2.age = 25;
                let utente3 = new Utente_1.Utente();
        ...
        return new Promise(async (resolve, reject) => {
            authenticate(SCOPES)
                ...
                docs.documents.get({
                    ...
                    console.log(JSON.stringify(res === null || res === void 0 ? void 0 : res.data.body, null, 4));
                });
            })
                .catch((err) => {
                console.log(err);
...
//# sourceMappingURL=srv-service-impl.js.map